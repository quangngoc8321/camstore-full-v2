...
const ReviewList = ({productId}) => {
  ...
  const fetchReviews = async (query) => {
      ...
      const option = {
        url:
          "/api/v1" +
          `/reviews?pageNumber=${pageNumber}&productId=${productId}`,
        method: "get",
      };
      return await axios(option);
    }; 
    
    const { isLoading, data } = useQuery(["reviews", query], () =>
    fetchReviews(query)
);
          ...
          renderItem={(item) => (
            ...
            <List.Item.Meta
              ...
              <>
                  ...
                  <Avatar style={{ backgroundColor: '#87d068' }} src={item.user.avatar.url} />
                </Tooltip>
                </>
              }
              title={<p style={{wordWrap:"break-word", wordBreak:"break-word",whiteSpace:"normal", textAlign:"start"}}><strong>{item.content}</strong> <br/>
              {formatDistance(new Date(item.createdAt), new Date(),{addSuffix: true})}
              </p>}
              description={
                <div>
                  <Rate allowHalf value={item.ratingPoint} disabled/> ({item.ratingPoint})
                </div>
              }
            />
          </List.Item>
